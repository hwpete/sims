...
async def call_openai(prompt: str, conversation: Optional[List[Dict[str, str]]] = None) -> Dict[str, Any]:
...
def parse_init(output: Dict[str, Any]) -> Dict[str, Any]:
    return {"world_background": output.get("world_background", "历史世界已建立。"),
            ...
            "suggested_actions": output.get("suggested_actions", ["观察所在地区", "拜访一位熟人", "处理眼前生计"])}
